...
int sha1sum(char *inbuf, int inlen, char *out, int len)
{
    int i;
    char *p = out;
    SHA1_CONTEXT ctx;
    sha1_init (&ctx);
    sha1_write (&ctx, inbuf, inlen);
    sha1_final (&ctx);
    for (i=0; i < 20; i++)
        p += sprintf_s(p, &out[len] - p, "%02x", ctx.buf[i]);
    return p - out;
}
...
//      exit (1);
        argc = 2;argv[1] = (char *)&"stdin";fp = stdin;
    }
